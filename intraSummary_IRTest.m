...
Files = dir('1195_112817_*_1_*_trace_*.txt'); % Find txt files containing this phrase to batch through
...
for ii = 1:length(Files)
    ...
    %%%%%%%%%%%% CHange how bg is calculated, so that only 50 ms window is used
...
%     title(testname,'Interpreter','none','Fontsize',12)
    ...
    title(['RMP = ', num2str(round(rmp,1)), 'mV', ...
        ' BG = ', num2str(round(mean(sum(Reps.raster(1:prestim*samplesPerMS-10, :))),1)), 'Hz', ...
