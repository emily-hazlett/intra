...
Files = dir('*3345_2*All_trace.txt'); % Find txt files containing this phrase to batch through
...
badrmp = 0; % Drop reps above this threshold
deltaV = 50; % change in V for figuring out which reps to drop
...
for ii = 1:length(Files)
    ...
    for pp = 1:length(stimList) %Save txt files for each stim
        ...
        if ~isempty(indTests) % Save a different txt file for each test
            ...
            for kk = 1:length(indTests)-1
                ...
                fid = fopen(txtname, 'w');
                fprintf(fid,[repmat('%s \t',1,length(texter)-1), '%s'],string(texterX));
                fprintf(fid,'\n');
                fclose(fid);
                dlmwrite(txtname, stimRepsX, '-append', 'delimiter', '\t','roffset', 0);
                count = count+1;
                
                clear daterSub texterSub
            end
